 ...
 *
 * Behaviour:
 * - Profile names (first_name, middle_name, last_name) are unique per project when composed; duplicates get suffix " (2)", " (3)", etc.
...
// Middle names (mother's maiden / second given name) - optional when seeding
const MIDDLE_NAMES = [
    'Dela Cruz', 'Santos', 'Reyes', 'Cruz', 'Garcia', 'Mendoza', 'Ramos', 'Torres', 'Fernandez', 'Gonzalez',
    'Diaz', 'Castillo', 'Sanchez', 'Flores', 'Rivera', 'Aquino', 'Villanueva', 'Bautista', 'Romero', 'Luna',
];
const SURNAMES = [
...
foreach ($existingRows as $row) {
    ...
    $usedNamesByProject[$pid][trim($row->full_name ?? '')] = true;
}
...
for ($batch = 0; $batch < $totalBatches; $batch++) {
    ...
    for ($i = 0; $i < $batchCount; $i++) {
        ...
        $middleName = random_int(0, 1) ? randomElement(MIDDLE_NAMES) : '';
        $lastName = randomElement(SURNAMES);
        $projectId = randomElement($projectIds);
        $baseName = trim($firstName . ' ' . $middleName . ' ' . $lastName);
