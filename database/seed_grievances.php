...
// Filipino names (first, middle optional, last/surname)
const MIDDLE_NAMES = [
    'Dela Cruz', 'Santos', 'Reyes', 'Cruz', 'Garcia', 'Mendoza', 'Ramos', 'Torres', 'Fernandez', 'Gonzalez',
];
const FIRST_NAMES = [
...
$defaultProgressLevels = GrievanceProgressLevel::defaults();
$defaultProgressLevelIds = array_values(array_map(fn($pl) => (int) $pl->id, $defaultProgressLevels));
$progressLevelIdsByProject = [];
foreach ($projectIds as $pid) {
    $rows = GrievanceProgressLevel::forProjectOrDefault((int) $pid);
    $ids = array_values(array_map(fn($pl) => (int) $pl->id, $rows));
    if (empty($ids)) {
        $ids = $defaultProgressLevelIds;
    }
    $progressLevelIdsByProject[(int) $pid] = $ids;
}
...
for ($i = 0; $i < $SEED_GRIEVANCE_COUNT; $i++) {
    ...
    if ($isPaps && !empty($profilesByProject[$projectId])) {
        ...
        $respondentFirstName = randomElement(FIRST_NAMES);
        $respondentMiddleName = random_int(0, 1) ? randomElement(MIDDLE_NAMES) : '';
        $respondentLastName = randomElement(SURNAMES);
        $respondentFullName = trim($respondentFirstName . ' ' . $respondentMiddleName . ' ' . $respondentLastName);
    ...
    // Keep seeded grievances compatible with normalized respondent autocomplete/history.
    $data['respondent_id'] = GrievanceRespondent::upsertFromGrievanceData($data);
    ...
    $currentLevel = null;
    $lastAt = $dateRecorded;
    $projectProgressLevelIds = $progressLevelIdsByProject[(int) $projectId] ?? $defaultProgressLevelIds;

    for ($t = 0; $t < $numTransitions; $t++) {
        if ($currentStatus === 'closed') break;
        $nextStatus = $statuses[array_rand($statuses)];
        if ($nextStatus === 'in_progress' && !empty($projectProgressLevelIds)) {
            $currentLevel = $projectProgressLevelIds[array_rand($projectProgressLevelIds)];
...
foreach ($grievanceIds as $gid) {
    if (random_int(0, 2) === 0) {
        $ch = json_encode(['respondent_first_name' => ['from' => 'Juan', 'to' => 'Juan'], 'respondent_last_name' => ['from' => 'Dela Cruz', 'to' => 'dela Cruz'], 'mobile_number' => ['from' => '09120000000', 'to' => '09181234567']]);
