...
<div class="card">
    <div class="card-body">
        <form method="post" action="<?= $item ? "/grievance/options/progress-levels/update/{$item->id}" : '/grievance/options/progress-levels/store' ?>">
            <?= \Core\Csrf::field() ?>
            <?php $selectedProjectId = isset($selectedProjectId) ? (int)$selectedProjectId : (int)($item->project_id ?? 0); ?>
            <div class="mb-3">
                <label class="form-label">Scope</label>
                <select name="project_id" class="form-select">
                    <option value="0" <?= $selectedProjectId === 0 ? 'selected' : '' ?>>Default (used when project has no own setup)</option>
                    <?php foreach (($projects ?? []) as $project): ?>
                    <option value="<?= (int)$project->id ?>" <?= $selectedProjectId === (int)$project->id ? 'selected' : '' ?>><?= htmlspecialchars($project->name) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
