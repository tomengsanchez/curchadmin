...
<div class="card mb-3">
    ...
    <div class="card-body">
        <dl class="row mb-0">
            ...
            <?php else: ?>
            <dt class="col-sm-3">Full Name</dt><dd class="col-sm-9"><?php
                $respondentName = trim(implode(' ', array_values(array_filter([
                    $g->respondent_first_name ?? '',
                    $g->respondent_middle_name ?? '',
                    $g->respondent_last_name ?? '',
                ], fn($v) => trim((string)$v) !== ''))));
                if ($respondentName === '') {
                    $respondentName = (string)($g->respondent_full_name ?? '');
                }
                echo htmlspecialchars($respondentName !== '' ? $respondentName : '-');
            ?></dd>
            <?php endif; ?>
