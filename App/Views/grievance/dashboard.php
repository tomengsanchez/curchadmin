...
(function() {
    ...
    function renderDashboard(d) {
        ...
        if (inProgContainer && Array.isArray(d.inProgressLevels)) {
            ...
            if (!d.inProgressLevels.length) {
                ...
                d.inProgressLevels.forEach(function(r) {
                    var levelDisplayName = r.display_level_name || r.level_name || "—";
                ...
                html += "</ul>";
                var needsRows = Array.isArray(d.needsEscalationRows) ? d.needsEscalationRows.filter(function(row) {
                ...
                if (needsRows.length) {
                    ...
                    html += "<ul class=\"list-unstyled mb-0\">";
                    needsRows.forEach(function(row) {
                        var label = (row.display_level_name || row.level_name || "Level") + " grievances need " + (row.action === "close" ? "to close" : "escalation");
                        html += "<li class=\"d-flex justify-content-between align-items-center py-1\">" +
                            "<span>" + label + "</span>" +
                            "<span class=\"badge bg-danger\">" + (row.count || 0).toLocaleString() + "</span>" +
                            "</li>";
                    });
                    html += "</ul>";
                }
        ...
        var inProgressForChart = Array.isArray(d.inProgressLevels) ? d.inProgressLevels.map(function(r) {
            return { name: r.display_level_name || r.level_name || "—", count: r.cnt || 0 };
        }) : [];
