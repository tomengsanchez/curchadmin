...
class Profile
{
    ...
    public static function listPaginated(string $search, array $searchColumns, string $sortBy, string $sortOrder, int $page, int $perPage, ?int $afterId = null, ?int $beforeId = null): array
    {
        ...
        $sql = "SELECT p.id, p.papsid, p.control_number, p.first_name, p.middle_name, p.last_name, p.full_name, p.age, p.contact_number, p.project_id,
    ...
    public static function all(): array
    {
        ...
        if ($allowed !== null) {
            ...
            $placeholders = implode(',', array_fill(0, count($allowed), '?'));
            $stmt = $db->prepare('
                SELECT p.id, p.papsid, p.control_number, p.first_name, p.middle_name, p.last_name, p.full_name, p.age, p.contact_number, p.project_id, proj.name as project_name
                FROM profiles p
                LEFT JOIN projects proj ON proj.id = p.project_id
    ...
    public static function create(array $data): int
    {
        ...
        $fullName = trim($data['full_name'] ?? trim(implode(' ', array_values(array_filter([$firstName, $middleName, $lastName], fn($v) => $v !== '')))));
        $stmt = self::db()->prepare('INSERT INTO profiles (
            papsid, control_number, first_name, middle_name, last_name, full_name, age, contact_number, project_id,
