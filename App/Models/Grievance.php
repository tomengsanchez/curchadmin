...
class Grievance
{
    ...
    public static function find(int $id): ?object
    {
        ...
        $stmt = $db->prepare("
            SELECT g.*,
                   COALESCE(r.is_paps, g.is_paps) AS is_paps,
                   COALESCE(r.profile_id, g.profile_id) AS profile_id,
                   COALESCE(NULLIF(TRIM(CONCAT_WS(' ', p.first_name, p.middle_name, p.last_name)), ''), p.full_name) as profile_name,
                   p.papsid, proj.name as project_name,
                   COALESCE(r.first_name, g.respondent_first_name) AS respondent_first_name,
                   COALESCE(r.middle_name, g.respondent_middle_name) AS respondent_middle_name,
                   COALESCE(r.last_name, g.respondent_last_name) AS respondent_last_name,
                   COALESCE(r.full_name, g.respondent_full_name) AS respondent_full_name,
            ...
            FROM grievances g
            LEFT JOIN grievance_respondents r ON r.id = g.respondent_id
            LEFT JOIN profiles p ON p.id = COALESCE(r.profile_id, g.profile_id)
    ...
    public static function listPaginated(string $search, array $searchColumns, string $sortBy, string $sortOrder, int $page, int $perPage, ?int $afterId = null, ?int $beforeId = null, array $filters = []): array
    {
        ...
        $sortCol = match ($sortBy) {
            ...
            'status' => 'g.status',
            'respondent_name' => "COALESCE(NULLIF(TRIM(CONCAT_WS(' ', p.first_name, p.middle_name, p.last_name)), ''), p.full_name, r.full_name, g.respondent_full_name)",
            'profile_name' => "COALESCE(NULLIF(TRIM(CONCAT_WS(' ', p.first_name, p.middle_name, p.last_name)), ''), p.full_name)",
            default => 'g.id',
        };
        ...
        if ($search !== '') {
            ...
            if (in_array('respondent_name', $searchColumns, true)) { $conds[] = "(COALESCE(NULLIF(TRIM(CONCAT_WS(' ', p.first_name, p.middle_name, p.last_name)), ''), p.full_name, r.full_name, g.respondent_full_name)" . $coll . ' LIKE ' . $likeParam . ')'; $params[] = $term; }
            if (in_array('profile_name', $searchColumns, true)) { $conds[] = "COALESCE(NULLIF(TRIM(CONCAT_WS(' ', p.first_name, p.middle_name, p.last_name)), ''), p.full_name)" . $coll . ' LIKE ' . $likeParam; $params[] = $term; }
            if (!empty($conds)) {
        ...
        $respondentIdFilter = isset($filters['respondent_id']) ? (int) $filters['respondent_id'] : 0;
        if ($respondentIdFilter > 0) {
            $whereCond .= ' AND g.respondent_id = ?';
            $params[] = $respondentIdFilter;
        }
        ...
        if ($needsEscalationFilter === '1') {
            ...
            // Use the start of the current status/level segment so note-only updates
            // with unchanged status/level do not reset escalation timing.
            $whereCond .= "
        ...
        $sql = "SELECT g.id, g.date_recorded, g.grievance_case_number, g.project_id, g.status, g.progress_level,
            COALESCE(r.profile_id, g.profile_id) AS profile_id,
            COALESCE(r.first_name, g.respondent_first_name) AS respondent_first_name,
            COALESCE(r.middle_name, g.respondent_middle_name) AS respondent_middle_name,
            COALESCE(r.last_name, g.respondent_last_name) AS respondent_last_name,
            COALESCE(r.full_name, g.respondent_full_name) AS respondent_full_name,
            COALESCE(NULLIF(TRIM(CONCAT_WS(' ', p.first_name, p.middle_name, p.last_name)), ''), p.full_name, r.full_name, g.respondent_full_name) as respondent_name,
            COALESCE(NULLIF(TRIM(CONCAT_WS(' ', p.first_name, p.middle_name, p.last_name)), ''), p.full_name) as profile_name,
        ...
        $countSql = "SELECT COUNT(*) FROM grievances g LEFT JOIN grievance_respondents r ON r.id = g.respondent_id LEFT JOIN profiles p ON p.id = COALESCE(r.profile_id, g.profile_id) LEFT JOIN projects proj ON proj.id = g.project_id WHERE 1=1 $whereCond $projectFilter";
    ...
    public static function create(array $data): int
    {
        ...
        $stmt = $db->prepare('INSERT INTO grievances (
            date_recorded, grievance_case_number, project_id, is_paps, profile_id, respondent_id, respondent_first_name, respondent_middle_name, respondent_last_name, respondent_full_name,
        ...
        $stmt->execute([
            ...
            (int) ($data['respondent_id'] ?? 0) ?: null,
            trim($data['respondent_first_name'] ?? ''),
            trim($data['respondent_middle_name'] ?? ''),
            trim($data['respondent_last_name'] ?? ''),
