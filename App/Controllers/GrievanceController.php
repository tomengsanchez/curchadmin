...
use App\Models\GrievanceRespondent;
...
class GrievanceController extends Controller
{
    ...
    public function index(): void
    {
        ...
        $filterRespondentId = isset($_GET['respondent_id']) ? (int) $_GET['respondent_id'] : 0;
        ...
        $pagination = Grievance::listPaginated(
            ...
            [
                ...
                'respondent_id' => $filterRespondentId,
        ...
        $progressLevels = $filterProjectId > 0
            ? GrievanceProgressLevel::forProjectOrDefault($filterProjectId)
        ...
        // (first log in current status/level segment), and which grievances are
        // still marked as "new" for the current user.
        $levelStartedAt = [];
        $unreadById = [];
        ...
        if (!empty($ids)) {
            ...
            $levelStartedAt = $this->computeCurrentInProgressLevelStartedAt($ids);
    ...
    /** Grievance respondent profile list (normalized respondent table). */
    public function respondents(): void
    {
        $this->requireCapability('view_grievance');
        $search = trim((string) ($_GET['q'] ?? ''));
        $page = max(1, (int) ($_GET['page'] ?? 1));
        $perPage = max(10, min(100, (int) ($_GET['per_page'] ?? 15)));
        $pagination = \App\Models\GrievanceRespondent::listPaginated($search, $page, $perPage);

        $this->view('grievance/respondents', [
            'respondents' => $pagination['items'],
    ...
    public function store(): void
    {
        ...
        $data['respondent_id'] = GrievanceRespondent::upsertFromGrievanceData($data, null);
    ...
    public function show(int $id): void
    {
        ...
        $progressLevels = GrievanceProgressLevel::forProjectOrDefault((int) ($grievance->project_id ?? 0));
        ...
        if (($grievance->status ?? 'open') === 'in_progress' && !empty($grievance->progress_level)) {
            $map = $this->computeCurrentInProgressLevelStartedAt([$id]);
            $levelKey = $id . '_' . (int) $grievance->progress_level;
            $levelStartedAt = $map[$levelKey] ?? null;
        }
    ...
    public function update(int $id): void
    {
        ...
        $existingRespondentId = isset($grievance->respondent_id) ? (int) $grievance->respondent_id : null;
        $data['respondent_id'] = GrievanceRespondent::upsertFromGrievanceData($data, $existingRespondentId);
    ...
    private function buildGrievanceChanges(object $grievance, array $data): array
    {
        ...
        $textFields = [
            'grievance_case_number', 'respondent_first_name', 'respondent_middle_name', 'respondent_last_name', 'respondent_full_name', 'gender', 'gender_specify',
    ...
    private function gatherGrievanceData(?object $grievance): array
    {
        ...
        $respondentFirstName = trim($_POST['respondent_first_name'] ?? '');
        $respondentMiddleName = trim($_POST['respondent_middle_name'] ?? '');
        $respondentLastName = trim($_POST['respondent_last_name'] ?? '');
        $respondentFullName = trim(implode(' ', array_values(array_filter([$respondentFirstName, $respondentMiddleName, $respondentLastName], fn($v) => $v !== ''))));
        if ($isPaps && $profileId) {
        ...
        return [
            ...
            'respondent_middle_name' => $respondentMiddleName,
    ...
    public function statusUpdate(int $id): void
    {
        ...
        if ($status === 'in_progress') {
            ...
            if ($pl > 0 && GrievanceProgressLevel::findForProjectOrDefault($pl, $projectId)) $progressLevel = $pl;
        }
    ...
    /**
     * Compute the start time of the current in-progress level segment.
     * Notes posted with unchanged status/level do not reset this timestamp.
     *
     * @param array<int,int> $grievanceIds
     * @return array<string,string> map key "grievanceId_progressLevelId" => created_at
     */
    private function computeCurrentInProgressLevelStartedAt(array $grievanceIds): array
    {
        $grievanceIds = array_values(array_unique(array_map('intval', $grievanceIds)));
        $grievanceIds = array_values(array_filter($grievanceIds, fn($id) => $id > 0));
        ...
        $sql = "
            SELECT id, grievance_id, status, progress_level, created_at
            FROM grievance_status_log
            WHERE grievance_id IN ($placeholders)
            ORDER BY grievance_id ASC, created_at ASC, id ASC
        ";
        $stmt = $db->prepare($sql);
        $stmt->execute($grievanceIds);
        $rows = $stmt->fetchAll(\PDO::FETCH_OBJ);

        $result = [];
        $currentGrievanceId = 0;
        $prevStatus = null;
        $prevLevel = null;
        $segmentStartedAt = null;

        foreach ($rows as $row) {
            $grievanceId = (int) ($row->grievance_id ?? 0);
            if ($grievanceId <= 0 || empty($row->created_at)) {
                continue;
            }

            $status = (string) ($row->status ?? '');
            $level = (int) ($row->progress_level ?? 0);
            $createdAt = (string) $row->created_at;

            if ($grievanceId !== $currentGrievanceId) {
            ...
            $isTransition = ($prevStatus === null) || ($status !== $prevStatus) || ($level !== $prevLevel);
            if ($isTransition) {
                $segmentStartedAt = $createdAt;
            }

            if ($status === 'in_progress' && $level > 0 && $segmentStartedAt !== null) {
                $result[$grievanceId . '_' . $level] = $segmentStartedAt;
            }
