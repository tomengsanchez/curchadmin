...
class ProfileController extends Controller
{
    ...
    private function buildProfilePayload(array $input, ?object $existing): array
    {
        ...
        return [
            ...
            'first_name' => trim((string) ($input['first_name'] ?? ($profile->first_name ?? ''))),
            'middle_name' => trim((string) ($input['middle_name'] ?? ($profile->middle_name ?? ''))),
            'last_name' => trim((string) ($input['last_name'] ?? ($profile->last_name ?? ''))),
            'full_name' => trim((string) ($input['full_name'] ?? trim(implode(' ', array_values(array_filter([
                trim((string) ($input['first_name'] ?? ($profile->first_name ?? ''))),
                trim((string) ($input['middle_name'] ?? ($profile->middle_name ?? ''))),
                trim((string) ($input['last_name'] ?? ($profile->last_name ?? ''))),
            ], fn($v) => $v !== '')))))),
            'age' => $age,
    ...
    {
        if (!$p) return null;
        $residingAttachments = $this->formatAttachments($p->residing_in_project_affected_attachments ?? '[]');
        $ownerAttachments = $this->formatAttachments($p->structure_owners_attachments ?? '[]');
        $ifNotOwnerAttachments = $this->formatAttachments($p->if_not_structure_owner_attachments ?? '[]');
        $ownElsewhereAttachments = $this->formatAttachments($p->own_property_elsewhere_attachments ?? '[]');
        $availedHousingAttachments = $this->formatAttachments($p->availed_government_housing_attachments ?? '[]');
        return [
            'id' => (int) $p->id,
            ...
            'availed_government_housing_attachments' => $availedHousingAttachments,
            'hh_income' => isset($p->hh_income) && $p->hh_income !== null ? (float) $p->hh_income : null,
        ];
    }

    /** Convert stored profile attachment JSON to link objects for API clients. */
    private function formatAttachments(?string $json): array
    {
        $paths = Profile::parseAttachments($json);
        $items = [];
        foreach ($paths as $path) {
            ...
            $items[] = [
                'name' => basename($path),
                'url' => \App\Controllers\ProfileController::attachmentUrl($path),
            ];
        }
        return $items;
    }
