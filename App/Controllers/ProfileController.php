...
class ProfileController extends Controller
{
    ...
    public function import(): void
    {
        ...
        // Minimal required logical fields: either full_name OR first_name/last_name pair.
        $hasFullName = array_key_exists('full_name', $colIndex);
        $hasNameParts = array_key_exists('first_name', $colIndex) && array_key_exists('last_name', $colIndex);
        if (!$hasFullName && !$hasNameParts) {
            fclose($fp);
            http_response_code(400);
            $this->json([
                'error' => 'MissingColumns',
                'message' => 'Import file must include either full_name, or first_name + last_name.',
            ]);
        }
        ...
        while (($row = fgetcsv($fp)) !== false) {
            ...
            if ($fullName === '') {
                $rowErrors[] = 'first_name + last_name (or full_name) is required.';
            }
